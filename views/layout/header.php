<!doctype html>
<html lang="en" dir="ltr">

<head>
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1, shrink-to-fit=no">
    <title data-setting="app_name" data-rightJoin=" Responsive Bootstrap 5 Admin Dashboard Template">Muzik Responsive Bootstrap 5 Admin Dashboard Template</title>
    <meta name="description" content="Muzik is a revolutionary Bootstrap Admin Dashboard Template and UI Components Library. The Admin Dashboard Template and UI Component features 8 modules.">
    <meta name="keywords" content="premium, admin, dashboard, template, bootstrap 5, clean ui, muzik, admin dashboard,responsive dashboard, optimized dashboard, simple auth">
    <meta name="author" content="Iqonic Design">
    <meta name="DC.title" content="Muzik Simple | Responsive Bootstrap 5 Admin Dashboard Template">
    <!-- Favicon -->
    <link rel="shortcut icon" href="https://templates.iqonic.design/muzik/html/assets/images/favicon.ico">

    <!-- Library / Plugin Css Build -->
    <link rel="stylesheet" href="/public/css/core/libs.min.css">

    <link rel="stylesheet" href="/public/vendor/sheperd/dist/css/sheperd.css">

    <!-- muzik Design System Css -->
    <link rel="stylesheet" href="/public/css/muzik.minf700.css?v=1.0.1">

    <!-- Custom Css -->
    <link rel="stylesheet" href="/public/css/custom.minf700.css?v=1.0.1">
    <!-- Dark Css -->
    <link rel="stylesheet" href="/public/css/dark.minf700.css?v=1.0.1">

    <!-- Customizer Css -->
    <link rel="stylesheet" href="/public/css/customizer.minf700.css?v=1.0.1">

    <!-- RTL Css -->
    <link rel="stylesheet" href="/public/css/rtl.minf700.css?v=1.0.1">

    <link rel="stylesheet" href="../assets/vendor/swiperSlider/swiper-bundle.min.css">

    <!-- Google Font -->
    <link rel="preconnect" href="https://fonts.googleapis.com/">
    <link rel="preconnect" href="https://fonts.gstatic.com/" crossorigin>
    <link rel="preconnect" href="https://fonts.googleapis.com/">
    <link rel="preconnect" href="https://fonts.gstatic.com/" crossorigin>
    <link href="https://fonts.googleapis.com/css2?family=Poppins:ital,wght@0,100;0,200;0,300;0,400;0,500;0,600;0,700;0,800;0,900;1,100;1,200;1,300;1,400;1,500;1,600;1,700&amp;display=swap" rel="stylesheet">
</head>

<body class=" ">
    <!-- loader Start -->
    <div id="loading">
        <div class="loader simple-loader">
            <div class="loader-body ">
                <img src="/public/images/loader.gif" alt="loader" class=" img-fluid ">
            </div>
        </div>
    </div>
    <!-- loader end -->